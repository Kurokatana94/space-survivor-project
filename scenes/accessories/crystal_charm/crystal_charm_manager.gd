...
func _ready():
	match level:
		0:
			instantiate_crystal_charm(crystal_charm_lq, 1, 0)
		1:
			instantiate_crystal_charm(crystal_charm_nm, 1, .1)
		2:
			instantiate_crystal_charm(crystal_charm_hq, 2, .25)

func instantiate_crystal_charm(crystal_charm: PackedScene, exp_bonus: float, exp_multiplier: float):
	var new_crystal_charm = crystal_charm.instantiate()
	add_child(new_crystal_charm)

	new_crystal_charm.exp_bonus = exp_bonus
